...
void LcdDisplay::SetupUI() {
    ...
    lv_obj_set_flex_flow(camera_hint_bar_, LV_FLEX_FLOW_ROW);
    ...
    camera_hint_label_ = lv_label_create(camera_hint_bar_);
    lv_label_set_text(camera_hint_label_, "双击退出，单击拍照");
    lv_obj_set_style_text_align(camera_hint_label_, LV_TEXT_ALIGN_LEFT, 0);

    lv_obj_set_style_text_color(camera_hint_label_, lvgl_theme->text_color(), 0);
    lv_obj_set_style_text_font(camera_hint_label_, text_font, 0);
...
void LcdDisplay::SetPreviewImage(std::unique_ptr<LvglImage> image) {
    ...
    // During preview, optionally show camera hint bar (only when enabled)
    if (bottom_bar_ != nullptr) {
