...
bool NeRtcProtocol::OpenAudioChannel(const std::string& wake_word) {
    int free_sram = heap_caps_get_free_size(MALLOC_CAP_INTERNAL);
    int min_free_sram = heap_caps_get_minimum_free_size(MALLOC_CAP_INTERNAL);
    ESP_LOGI(TAG, "NeRtcProtocol OpenAudioChannel, wake_word:%s [free_sram: %u min_free_sram: %u]", wake_word.c_str(), free_sram, min_free_sram);
