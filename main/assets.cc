...
bool Assets::Apply() {
    ...
    if (cJSON_IsString(font)) {
        ...
        if (GetAssetData(fonts_text_file, ptr, size)) {
            // Copy font data to RAM/PSRAM to avoid cache coherency issues
            // when accessed from LVGL rendering task on Core 1
            void* font_data_copy = heap_caps_malloc(size, MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL);
            if (font_data_copy == nullptr) {
                // Try PSRAM if internal RAM fails
                font_data_copy = heap_caps_malloc(size, MALLOC_CAP_8BIT | MALLOC_CAP_SPIRAM);
            }
            if (font_data_copy == nullptr) {
                ESP_LOGE(TAG, "Failed to allocate memory for font data (%zu bytes)", size);
                return false;
            }
            memcpy(font_data_copy, ptr, size);
            
            // Test font creation first without ownership
            // If it fails, we can free the memory immediately
            {
                LvglCBinFont test_font(font_data_copy, false);
                if (test_font.font() == nullptr) {
                    ESP_LOGE(TAG, "Failed to load fonts.bin");
                    heap_caps_free(font_data_copy);
                    return false;
                }
            }
            
            // Font creation succeeded, now create with ownership
            auto text_font = std::make_shared<LvglCBinFont>(font_data_copy, true);
...
bool Assets::Download(std::string url, std::function<void(int progress, size_t speed)> progress_callback) {
    ...
    // 在取消内存映射之前，先清理显示系统中使用的资源，避免访问已释放的内存
#ifdef HAVE_LVGL
    auto display = Board::GetInstance().GetDisplay();
    if (display != nullptr) {
        // 获取当前主题并清除背景图片等资源
        ...
        if (theme != nullptr) {
            ...
            if (lvgl_theme != nullptr) {
                // 清除背景图片，避免 LVGL 渲染任务访问无效内存
                lvgl_theme->set_background_image(nullptr);
            }
        }
        
        // 锁定显示，确保 LVGL 渲染任务完成当前帧并停止访问资源
        {
            DisplayLockGuard lock(display);
            // 等待一小段时间，确保渲染任务完成当前操作
